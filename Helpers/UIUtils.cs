...
namespace MOAR.Helpers
{
    ...
    public static class UIUtils
    {
        ...
        /// <returns>True if the shortcut is actively being held down.</returns>
        public static bool BetterIsPressed(this KeyboardShortcut shortcut)
        {
            if (shortcut.MainKey == KeyCode.None)
                return false;

            return Input.GetKey(shortcut.MainKey) && AreModifiersHeld(shortcut);
        }
        ...
        /// </summary>
        /// <param name="shortcut">The keyboard shortcut configuration.</param>
        /// <returns>True if the shortcut was just triggered this frame.</returns>
        public static bool BetterIsDown(this KeyboardShortcut shortcut)
        {
            if (shortcut.MainKey == KeyCode.None)
                return false;

            return Input.GetKeyDown(shortcut.MainKey) && AreModifiersHeld(shortcut);
        }
        ...
        /// <returns>True if no modifiers are defined, or if all modifiers are currently held.</returns>
        private static bool AreModifiersHeld(KeyboardShortcut shortcut)
        {
            return shortcut.Modifiers is null || shortcut.Modifiers.All(Input.GetKey);
        }
    }
}
